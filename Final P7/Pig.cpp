...
namespace cs31
{
    ...
    {
		int amountrolled = mHuman.Player::roll(amount);
    ...
    {
		int amountrolled = mComputer.Player::roll(amount);
    ...
    bool Pig::isGameOver()
    {
		...
		{
			mBoard.Board::markHumanAsWinner();
			return (true);
		}
		else if (Pig::determineGameOutcome() == COMPUTERWONGAME)
		{
			mBoard.Board::markComputerAsWinner();
			return (true);
		}
        return( false );
    }
